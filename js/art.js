...
/* ---------------- Kai ---------------- */
// s = full standing height in px. Anchor (x, y) = feet position.
export function drawKai(ctx, x, y, s, pose = 'stand', o = {}) {
  const lw = Math.max(2.5, s * 0.05);
  const hr = s * 0.14; // head radius
  ctx.save();
  ctx.translate(x, y);

  const head = (hx, hy, awake = true, dir = 0) => {
    ctx.beginPath();
    ctx.fillStyle = PAPER;
    ctx.strokeStyle = INK;
    ctx.lineWidth = lw;
    ...
    ctx.fillStyle = INK;
    for (let i = 0; i < 5; i++) {
      const a = -Math.PI * 0.85 + (i / 4) * Math.PI * 0.7 + dir * 0.2;
      const bx1 = hx + Math.cos(a - 0.18) * hr * 0.95;
      const by1 = hy + Math.sin(a - 0.18) * hr * 0.95;
      const bx2 = hx + Math.cos(a + 0.18) * hr * 0.95;
      const by2 = hy + Math.sin(a + 0.18) * hr * 0.95;
      const tx = hx + Math.cos(a) * hr * 1.65;
      const ty = hy + Math.sin(a) * hr * 1.65;
      ctx.beginPath();
      ctx.moveTo(bx1, by1);
      ctx.lineTo(tx, ty);
      ctx.lineTo(bx2, by2);
    ...
    const ex = hx + dir * hr * 0.3;
    if (awake) {
      ctx.beginPath();
      ctx.moveTo(ex - hr * 0.42, hy);
      ctx.lineTo(ex - hr * 0.42, hy + hr * 0.28);
      ctx.moveTo(ex + hr * 0.3, hy);
      ctx.lineTo(ex + hr * 0.3, hy + hr * 0.28);
      ctx.stroke();
    } else {
      ctx.beginPath();
      ctx.moveTo(ex - hr * 0.55, hy + hr * 0.1);
      ctx.lineTo(ex - hr * 0.2, hy + hr * 0.1);
      ctx.moveTo(ex + hr * 0.15, hy + hr * 0.1);
      ctx.lineTo(ex + hr * 0.5, hy + hr * 0.1);
      ctx.stroke();
    }
  };

  const L = (pts) => stroke(ctx, pts.map(([px, py]) => [px * s, py * s]), lw);

  switch (pose) {
    case 'lying':
      // horizontal on the ground, head to the left
      head(-s * 0.32, -hr * 0.9, o.awake ?? false, 0);
      L([[-0.2, -0.1], [0.12, -0.1]]);                    // torso
      L([[-0.14, -0.1], [-0.02, -0.02]]);                 // arm
      L([[0.12, -0.1], [0.28, -0.04]]);                   // leg 1
      L([[0.12, -0.1], [0.3, -0.12]]);                    // leg 2
      break;
    case 'sit':
      head(0, -s * 0.52, o.awake ?? true, 0);
      L([[0, -0.4], [0, -0.18]]);                         // torso
      L([[0, -0.32], [-0.16, -0.14]]);                    // arm back (propping)
      L([[0, -0.32], [0.14, -0.2]]);                      // arm
      L([[0, -0.18], [0.2, -0.16], [0.24, 0]]);           // bent leg
      L([[0, -0.18], [0.3, -0.02]]);                      // leg
      break;
    case 'stand':
      head(0, -s * 0.86, true, o.dir ?? 0);
      L([[0, -0.72], [0, -0.36]]);                        // torso
      L([[0, -0.62], [-0.14, -0.42]]);                    // arm
      L([[0, -0.62], [0.14, -0.44]]);                     // arm
      L([[0, -0.36], [-0.1, 0]]);                         // leg
      L([[0, -0.36], [0.1, 0]]);                          // leg
      break;
    case 'touch': // reaching out to the right (panel border)
      head(s * 0.04, -s * 0.86, true, 1);
      L([[0, -0.72], [-0.02, -0.36]]);
      L([[0, -0.64], [0.3, -0.6]]);                       // reaching arm
      L([[0, -0.6], [-0.12, -0.42]]);
      L([[-0.02, -0.36], [-0.12, 0]]);
      L([[-0.02, -0.36], [0.08, 0]]);
      break;
    case 'crouch':
      head(0, -s * 0.5, true, o.dir ?? 0);
      L([[0, -0.38], [0.02, -0.2]]);
      L([[0, -0.32], [-0.12, -0.18]]);
      L([[0, -0.32], [0.12, -0.2]]);
      L([[0.02, -0.2], [-0.1, -0.1], [-0.08, 0]]);
      L([[0.02, -0.2], [0.14, -0.1], [0.12, 0]]);
      break;
    case 'dodge': { // lunging toward o.dir (-1 left / +1 right)
      const d = o.dir ?? 1;
      head(d * s * 0.24, -s * 0.6, true, d);
      L([[d * 0.06, -0.5], [d * 0.2, -0.48]]);            // leaning torso
      L([[d * 0.1, -0.5], [d * 0.34, -0.56]]);            // arm fwd
      L([[d * 0.1, -0.5], [-d * 0.06, -0.36]]);           // arm back
      L([[d * 0.2, -0.48], [d * 0.34, -0.24], [d * 0.4, 0]]); // front leg
      L([[d * 0.2, -0.48], [-d * 0.04, -0.2], [-d * 0.1, 0]]); // trailing leg
      break;
    }
    case 'fight': { // club raised
      const d = o.dir ?? 1;
      head(0, -s * 0.84, true, d);
      L([[0, -0.7], [0, -0.34]]);
      L([[0, -0.62], [d * 0.18, -0.78]]);                 // club arm up
      L([[0, -0.58], [-d * 0.12, -0.44]]);
      L([[0, -0.34], [-0.12, 0]]);
      L([[0, -0.34], [d * 0.14, 0]]);
      // letter-C club in raised hand
      drawLetterC(ctx, d * s * 0.26, -s * 0.94, s * 0.16);
      break;
    }
    case 'hold-club': {
      const d = o.dir ?? 1;
      head(0, -s * 0.86, true, d);
      L([[0, -0.72], [0, -0.36]]);
      L([[0, -0.62], [d * 0.2, -0.52]]);
      L([[0, -0.62], [-d * 0.12, -0.44]]);
      L([[0, -0.36], [-0.1, 0]]);
      L([[0, -0.36], [d * 0.1, 0]]);
      drawLetterC(ctx, d * s * 0.28, -s * 0.52, s * 0.14);
      break;
    }
  }
  ctx.restore();
...
export function drawLetterC(ctx, x, y, size) {
  ctx.save();
  ctx.font = `900 ${size * 2}px -apple-system, "Arial Black", sans-serif`;
  ctx.textAlign = 'center';
  ctx.textBaseline = 'middle';
  ctx.lineJoin = 'round';
  ctx.strokeStyle = INK;
